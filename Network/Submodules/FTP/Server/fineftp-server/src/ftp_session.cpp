...
namespace fineftp
{
	...
	FtpSession::FtpSession(asio::io_service& io_service, const std::shared_ptr<UserDatabase>& user_database, const std::string ftp_root_directory, const std::function<void()>& completion_handler)
		...
		, ftp_root_directory_(ftp_root_directory)
	...
	FtpMessage FtpSession::handleFtpCommandCWD(const std::string& param)
	{
		...
		if (!hasObjectPermissions(param, FilePermission::DirList)) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
	...
	{
		if (!logged_in_user_)                                                                return FtpMessage(FtpReplyCode::NOT_LOGGED_IN, "Not logged in");
		if (!hasObjectPermissions(param, FilePermission::FileRead)) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
	...
	FtpMessage FtpSession::handleFtpCommandSTOR(const std::string& param)
	{
		...
		if (static_cast<int>(logged_in_user_->user_permissions_ & UserPermission::FileWrite) == 0) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
		if (!data_acceptor_.is_open())                                                             return FtpMessage(FtpReplyCode::ERROR_OPENING_DATA_CONNECTION, "Error opening data connection");
		...
		if (existing_file_filestatus.isOk())
		{
			if ((existing_file_filestatus.type() == Filesystem::FileType::RegularFile)
				&& !hasObjectPermissions(param, FilePermission::FileDelete))
			{
	...
	{
		if (!logged_in_user_)                                         return FtpMessage(FtpReplyCode::NOT_LOGGED_IN, "Not logged in");
		if (!hasObjectPermissions(param, FilePermission::FileAppend)) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
		if (!data_acceptor_.is_open())                                return FtpMessage(FtpReplyCode::ERROR_OPENING_DATA_CONNECTION, "Error opening data connection");
	...
	FtpMessage FtpSession::handleFtpCommandDELE(const std::string& param)
	{
		...
		{
			if (!hasObjectPermissions(param, FilePermission::FileDelete))
			{
	...
	{
		if (!logged_in_user_)                                        return FtpMessage(FtpReplyCode::NOT_LOGGED_IN, "Not logged in");
		if (!hasObjectPermissions(param, FilePermission::DirDelete)) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
	...
	{
		if (!logged_in_user_)                                                                      return FtpMessage(FtpReplyCode::NOT_LOGGED_IN, "Not logged in");
		if (static_cast<int>(logged_in_user_->user_permissions_ & UserPermission::DirCreate) == 0) return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
	...
	std::string FtpSession::toLocalPath(const std::string& ftp_path) const
	{
		...
		return fineftp::Filesystem::cleanPathNative(ftp_root_directory_ + "/" + absolute_ftp_path);
	}
	...
	FtpMessage FtpSession::checkIfPathIsRenamable(const std::string& ftp_path) const
	{
		...
		if (!ftp_path.empty())
		{
			...
			if (file_status.isOk())
			{
				// Calculate required permissions to rename the given resource and send error if the permisions are insufficient
				if (file_status.type() == Filesystem::FileType::Dir)
				{
					if (!hasObjectPermissions(ftp_path, FilePermission::DirRename))
						return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
				}
				else
				{
					if (!hasObjectPermissions(ftp_path, FilePermission::FileRename))
						return FtpMessage(FtpReplyCode::ACTION_NOT_TAKEN, "Permission denied");
				}
