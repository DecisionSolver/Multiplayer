...
namespace fineftp
{
	FtpServerImpl::FtpServerImpl(const std::string& address, uint16_t port, const std::string& ftp_working_directory,
		...
		const unsigned short& DBport, const std::string& DBcharset, bool DBOnlyRead)
		: port_(port)
		, address_(address)
		...
		, open_connection_count_(0)
	{
		...
		if (!ftp_users_->Connect(DBuser, DBpassword, DBhost, DB, Table, DBport, DBcharset, DBOnlyRead))
	...
	bool FtpServerImpl::addNewUser(const std::string& username, const std::string& password,
	...
	{
		auto ftp_session = std::make_shared<FtpSession>(io_service_, ftp_users_, ftp_working_directory_,
		...
		std::cout << "FTP Server created." << std::endl << "Listening at address " <<
			acceptor_.local_endpoint().address() << " on port " << acceptor_.local_endpoint().port() << ":" << std::endl;
#endif // NDEBUG
	...
	void FtpServerImpl::acceptFtpSession(std::shared_ptr<FtpSession> ftp_session, asio::error_code const& error)
	{
		...
		auto new_session = std::make_shared<FtpSession>(io_service_, ftp_users_, ftp_working_directory_,
			[this]() { open_connection_count_--; });
