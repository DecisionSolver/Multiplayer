...
bool FindSubStr(std::wstring context, std::wstring const from)
{
...
bool FindSubStr(std::string context, std::string const from)
{
	if (context.empty() || from.empty())
		return false;
...
void getFloat3Text(std::string context, const std::string &Char2Split, std::vector<float> &Float3)
{
...
void getTextFloat3(std::string &context, const std::string &Char2Split, std::vector<float> Float3)
{
	std::stringstream stream;
	for (size_t i = 0; i < Float3.size(); i++)
	{
		stream << std::fixed << std::setprecision(3) << Float3.at(i);
