...
{
	Points.push_back(std::make_shared<Point>(Vector3::Zero, -Vector3::Up, WalkToNewPoint));
	Points.push_back(std::make_shared<Point>(Vector3::One, Vector3::Forward, WalkToNewPoint));
	Points.push_back(std::make_shared<Point>(Vector3(2, 2, 2), Vector3::Left * 2.f, WalkToNewPoint));
	Points.push_back(std::make_shared<Point>(Vector3(3, 3, 3), -Vector3::Backward, WalkToNewPoint));
}
...
void SimpleLogic::Update(Vector3 &Pos, Vector3 &Rot, float dTime)
{
	...
	if (_Point->GetState() != Stay)
	{
		...
		if (DirectX::XMVector3NearEqual(_Point->GetPos(), Pos, Vector3(0.001f, 0.001f, 0.001f)) &&
			DirectX::XMVector3NearEqual(_Point->GetRotate(), Rot, Vector3(0.001f, 0.001f, 0.001f)))
			Progress++;
	}
	else
		Progress++;
