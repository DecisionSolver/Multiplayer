...
public:
	...
	public:
		...
		const std::string ID; // Only ID Of Node
...
public:
	...
	long Load(const std::string &FileBuff);
	long Load(const std::shared_ptr<tinyxml2::XMLDocument> &NewDoc);
	void Process();
	void Update();
