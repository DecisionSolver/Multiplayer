...
public:
	...
	struct Point
	{
		...
		void SetPos(const Vector3 &Position) { Pos = Position; }
		void SetRotate(const Vector3 &Rotation) { Rotate = Rotation; }
		void SetState(LogicMode State) { TestState = State; }
	private:
		Vector3 Pos = { 0, 0, 0 }, Rotate = { 0, 0, 0 };
		LogicMode TestState;
	};
...
private:
	size_t Progress = 0;
	bool isPaused = false;
	LogicMode CurrentModes = LogicMode::Stay;
