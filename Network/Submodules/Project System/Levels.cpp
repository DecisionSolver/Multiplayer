...
long Level::Load(const std::shared_ptr<tinyxml2::XMLDocument> &NewDoc)
{
	if (!doc)
		doc = std::make_shared<tinyxml2::XMLDocument>(&*NewDoc);
	else
		doc = NewDoc;
	Process();
...
long Level::Load(const std::string &FileBuff)
{
	if (FileBuff.empty())
	{
...
void Level::Process()
{
	...
	if (IsModels)
	{
		for (;;)
		{
			...
			if (models && models->NoChildren()) break; // <models/>
			if (Models.empty() && models->FirstChildElement())
	...
	if (IsSobjs)
	{
		for (;;)
		{
			if (s_objs && s_objs->NoChildren()) break; // <s_objs/>
			if (S_objs.empty() && s_objs->FirstChildElement())
	...
	for (auto It : Models)
	{
		...
		for (;;)
		{
			...
			if (FirstAttr)
			{
				...
				{
					if (strcmp(FirstAttr->Value(), _ATTR_MARK_REMOVED_) == 0)
						IsRemoved = true;
					if (strcmp(FirstAttr->Value(), _ATTR_MARK_ADDED_) == 0)
						IsAdded = true;
				}
		...
		else
		{
			...
			if (std::filesystem::exists(FS->getPathFromType(_TypeOfFile::MODELS) + ModelFileName))
			{
			...
			if (NewNode && !It->NoChildren()) // If Have <logic>
			{
				...
				std::shared_ptr<SimpleLogic> NewLogic = std::make_shared<SimpleLogic>();
...
#if defined (DS_Engine)
#if defined(without_multiplayer)
	const_cast<std::string &>(nd->ID) = to_std::string(MainChild->GetNodes().size());
#else
	const_cast<std::string &>(const_cast<std::string &>(nd->ID)) = md5_from_file(PathModel);
#endif
#else
	const_cast<std::string &>(nd->ID) = md5_from_file(PathModel);
#endif
	nd->RenderName = NodeName.empty() ? std::filesystem::path(PathModel).filename().string() : NodeName;
	nd->GM = std::make_shared<GameObject::Object>(nd->ID, std::filesystem::path(PathModel).filename().string(),
		nullptr, Model, Pos, Scale, Rotate);
	if (!nd->GM || (nd->GM && nd->GM->GetIdText().empty()))
		return std::make_shared<Level::Node>();
...
std::shared_ptr<Level::Node> Level::Add(const std::shared_ptr<GameObject::Object> &GM)
{
	...
	const_cast<std::string &>(nd->ID) = GM->GetIdText();
...
std::string Level::Save(const std::shared_ptr<tinyxml2::XMLDocument> &Doc, const std::shared_ptr<Node> &Node)
{
	...
	if (Node->SaveInfo->T == GameObject::TYPE::Sound_Obj)
	{
		...
		else if (scene->FirstChildElement(_SOUND_OBJECTS_))
			s_objs = scene->FirstChildElement(_SOUND_OBJECTS_);
	}
	...
	if (Node->SaveInfo->T == GameObject::TYPE::Sound_Obj && !s_obj)
	{
		...
		tmp->SetAttribute(_ATTR_FILE_NAME_,
			std::filesystem::path(Node->GM->GetModelNameFile()).filename().string().c_str());
		...
		for (;;) // Count Of Nodes
		{
				...
				->ToElement()->FindAttribute(_ATTR_FILE_NAME_))
			{
				if (!(Node->SaveInfo->T == GameObject::TYPE::Sound_Obj ? s_obj : model)
					...
					FirstAttr->SetAttribute(std::filesystem::path(Node->GM->GetModelNameFile()).filename()
						.string().c_str());
			}
			...
			if (Node->SaveInfo->Scale && nameNode == _ATTR_SCALE_)
			{
				Pass.clear();
				Result.clear();
				Pass.push_back(Node->GM->GetScaleCord()[0]);
				Pass.push_back(Node->GM->GetScaleCord()[1]);
				Pass.push_back(Node->GM->GetScaleCord()[2]);
...
void Level::Child::Update()
{
	for (size_t i = 0; i < Nodes.size(); i++)
	{
		...
		ToDo("Update logic time with time of server (like timeframe in Render)")
		it->UpdateLogic(1);
		/*if (Model)
		{
...
{
	std::shared_ptr<tinyxml2::XMLDocument> OneDoc = std::make_shared<tinyxml2::XMLDocument>();
