...
void File_system::SetCallbackFileAction(const std::function<void(FW::WatchID watchid, const FW::String &dir,
	const FW::String &filename, FW::Action action)> &Callback)
{
...
File_system::File_system()
{
	...
	auto PathBuf = _getcwd(nullptr, 0);
	if (!PathBuf)
	{
#if __has_include("logger.h")
...
void File_system::AddFolderToWatch(const std::string &FolderToWatch)
{
	try
	{
		fileWatcher.addWatch(FolderToWatch, &listener);
	}
...
{
	SetCallbackFileAction(Callback);
	AddFolderToWatch(FolderToWatch);
}
...
void File_system::ScanFiles()
{
	...
	{
#if __has_include("logger.h")
		Logger_Error("Resource Folder Was Not Find! Try To Create Folder");
#endif
		try
		{
			std::filesystem::create_directory(WorkDirSources);
		}
		catch (const std::exception &exception)
		{
			Logger_Fatal_F("Engine Cannot Work Without Resource Folder! Error: {}", exception.what());
		}
	}
	...
	for (size_t i = 0; i < file.size(); i++)
	{
		...
		{
		case MODELS:
			Models.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case TEXTURES:
			Textures.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case LEVELS:
			Levels.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case DIALOGS:
			Dialogs.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case SOUNDS:
			Sounds.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case SHADERS:
			Shaders.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));

			c_AllFilesWasAdded++;
			break;
		case UIS:
			Uis.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			
			c_AllFilesWasAdded++;
			break;
		case SCRIPTS:
			Scripts.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));

			c_AllFilesWasAdded++;
			break;
		case FONTS:
			Fonts.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));

			c_AllFilesWasAdded++;
			break;
		default:
			None.emplace_back(make_pair(make_shared<File_system::File>(someFile, ext,
				Fname, (size_t)file_size(someFile), type, false, Hash), file.at(i)));
			break;
		}
	}
...
_TypeOfFile File_system::GetTypeFileByExt(const path &File)
{
	...
	{
		if (!File.has_parent_path() && !File.has_root_directory() &&
...
shared_ptr<File_system::File> File_system::Find(const path &File, bool AlsoAddFile)
{
	...
	if (AlsoAddFile)
	{
		...
		for (size_t i = 0; i < AllFiles.size(); i++)
		{
			...
			auto this_file = AllFiles[i].string();
			boost::to_lower(this_file);
			const path Files = this_file;
			// Models
			if (((!File.has_root_path()) || !File.has_extension()) ||
...
shared_ptr<File_system::File> File_system::OnlyAddFile(const path &File)
{
	...
	std::filesystem::copy(File, pathType + path(pathType).filename().string());
	...
	case PROJECT:
		break;
	case MODELS:
		Models.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Models.back().first;
		break;
	case TEXTURES:
		Textures.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Textures.back().first;
		break;
	case LEVELS:
		Levels.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Levels.back().first;
		break;
	case DIALOGS:
		Dialogs.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Dialogs.back().first;
		break;
	case SOUNDS:
		Sounds.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Sounds.back().first;
		break;
	case SHADERS:
		Shaders.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Shaders.back().first;
		break;
	case UIS:
		Uis.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Uis.back().first;
		break;
	case SCRIPTS:
		Scripts.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
		return Scripts.back().first;
		break;
	case FONTS:
		Fonts.emplace_back(make_pair(make_shared<File_system::File>(File.string(), File.extension().string(),
			File.filename().string(), (size_t)file_size(File), T, false, Hash), File.string()));
...
vector<path> File_system::getFilesInFolder(const path &Folder, bool Recursive, bool onlyFile)
{
	...
	if (!Recursive && !onlyFile)
		for (directory_iterator it(Folder); it != directory_iterator(); ++it)
		{
			auto str = it->path();
			if (is_directory(str))
				files.push_back(str.lexically_normal());
		}
...
bool File_system::ReadFileMemory(const char *filename, size_t &FileSize,
	vector<unsigned char> &FilePtr)
{
...
{
	path p = (GetCurrentPath() + "Workspace/settings.cfg");
	try
	{
		if (!exists(p))
		{
#if __has_include("logger.h")
			Logger_Warn_F("File Settings: \"{}\" Did Not Find! Load Settings Will Skip.", p.string().c_str());
#endif
	...
	const FW::String &filename, FW::Action action)
{
	if (Callback)
	{
		Callback(watchid, dir, filename, action);
	}
}
