...
void ProjectFile::OpenOrCreateDB(bool ifNeedConnect)
{
	...
	std::string DriverString = "Microsoft Access Driver (*.mdb)";
...
std::shared_ptr<Packet> Info = std::make_shared<Packet>();
nlohmann::json JData;
//
	...
	const Connection::SharedPtr &User, const network::Packet::Type &Type)
{
			...
			: true)))
	{
		...
		for (size_t i = AllData["Hash Commit"].size() - 1; i >= 0; i--)
		{
				...
				(nlohmann::json::value_type)ID_Commit))
			{
					...
					// Get String Here 'Cause All Data Came Only That Type
				{
					...
					else if (User && !PermToLoadProj)
					{
						...
						std::string text = "You're Not Admin and don't have permissions to load project on server";

						if (!ThisLevel->IsLoaded())
						{
							text += "And Server Doesn't Have Loaded Project!";
						}

						Pack->CreatePacket((int)Type, true,
							{
								{
									"Err",
									text,
					...
					if (User)
					{
						...
						if (Type == Packet::Type::Get_MetaData_Project_Ex)
						{
							for (const auto &It: Nodes)
							{
								...
								getTextFloat3(Pos, ",", GM->GetPositionCord());
								getTextFloat3(Rot, ",", GM->GetRotCord());
								getTextFloat3(Scl, ",", GM->GetScaleCord());

								JData["_0"].push_back({ { "ID", It->ID }, {"RName", It->RenderName },
						...
						if (!JData.empty() && JData.dump() != "null")
						{
							Info->CreatePacket((int)Type, true, JData);
...
void ProjectFile::CreateProject(const std::string &Name)
{
	std::string BuffCmp;
	{
		...
		if (ConvertTo->Parse(("<?xml version=\"1.0\" encoding=\"UTF-8\"?>\n" +
			std::string(_SCENE_BEGIN) + "\n" + std::string(_SCENE_END) + "\n").c_str()) == XML_SUCCESS)
		{
			auto Compressed = nlohmann::json::to_msgpack(XMLtoJSON(ConvertTo->FirstChildElement()));
			BuffCmp = std::string(Compressed.begin(), Compressed.end());
		}
		delete ConvertTo;
	}
	...
	if (!BuffCmp.empty())
	{
		...
		std::string Date = std::string(timedisplay, len), Hash = md5_from_buffer(Date + BuffCmp);
...
{
	const_cast<std::string &>(CurrentProj) = Name;
}
