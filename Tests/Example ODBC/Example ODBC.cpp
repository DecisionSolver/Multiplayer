...
std::pair<bool, std::vector<std::string>> Test(SQLHSTMT hStmt)
{
...
int __cdecl wmain(int argc, _In_reads_(argc) WCHAR **argv)
{
	...
	std::string DriverString = "Driver={Microsoft Access Driver (*.mdb)};Dbq=G:/DecisionSolver/Engine/Workspace/resource/all projects.mdb;";
	...
	while (_fgetts(wszInput, SQL_QUERY_SIZE - 1, stdin))
	{
		...
		if (std::wstring(wszInput) == L"test\n")
		{
			Test(hStmt);
			continue;
		}
