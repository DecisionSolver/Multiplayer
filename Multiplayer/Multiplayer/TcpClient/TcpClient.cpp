...
namespace mp
{
	...
	void TcpClient::MessageHandler(TcpClient* client)							   //
	{
		...
		{
			if (client->unread_letters.size() < client->settings.max_number_of_letters)
			{
	...
	//////////////////////////////////////////////////////////////////////////////////////////////////////////////
	net::Socket::Status TcpClient::Send(const Message message, const DWORD recipient_id, Answer& answer) const  //
	{
		Sleep(1);
		...
		net::Socket::Status _status[] = { socket.Send(message.packet), socket.Send(_message_info) };//, socket.Receive(_answer)};
